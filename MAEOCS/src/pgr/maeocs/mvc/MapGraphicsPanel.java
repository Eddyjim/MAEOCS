...
/**
 * Componente grafico para Swing que tiene la capacidad de representar el
 * mapa y su respectiva grilla.
 * 
 * @author Carlos Gaitan
 * @author Edward Jimenez
 */
public class MapGraphicsPanel extends JPanel{
    private static final long serialVersionUID = -1578564103758995215L;

    private int width=0;
    private int height=0;

    /*
     * Dimensiones de cada secci�n, en pixeles
     */
    ...
    public static MapGraphicsPanel getInstance (int widthX, int heightY, int TamGrid,
    		SectionSelectedState stateI, DirGraphicsPanel dir){
    	return new MapGraphicsPanel (widthX,heightY,TamGrid,stateI, dir);
    }
    ...
    private void createGrid(){
    	...
    	for (int i = 0; i < gridTam; i++) {
			for (int j = 0; j < gridTam; j++) {
				mapSections[i][j]= new MapSection(state,diri);
				mapSections[i][j].setType(SectionType.NULL);
